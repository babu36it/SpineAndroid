...
class AudioRecordView : View {
    ...
    private var chunkAlignTo = AlignsTo.CENTER
...
//    }
    ...
    private fun init(attrs: AttributeSet) {
        val a: TypedArray = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.AudioRecordViews, 0, 0)
        try {
            chunkSpace = a.getDimension(R.styleable.AudioRecordViews_chunkSpace, chunkSpace)
            chunkMaxHeight = a.getDimension(R.styleable.AudioRecordViews_chunkMaxHeight, chunkMaxHeight)
            chunkMinHeight = a.getDimension(R.styleable.AudioRecordViews_chunkMinHeight, chunkMinHeight)
            setChunkRoundedCorners(a.getBoolean(R.styleable.AudioRecordViews_chunkRoundedCorners, chunkRoundedCorners))
            setChunkWidth(a.getDimension(R.styleable.AudioRecordViews_chunkWidth, chunkWidth))
            setChunkColor(a.getColor(R.styleable.AudioRecordViews_chunkColor, chunkColor))
            val var5 = a.getInt(R.styleable.AudioRecordViews_chunkAlignsTo, chunkAlignTo.ordinal)
            chunkAlignTo = AlignsTo.BOTTOM
            chunkSoftTransition = a.getBoolean(R.styleable.AudioRecordViews_chunkSoftTransition, chunkSoftTransition)
