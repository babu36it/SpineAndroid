...
class PodcastDetailActivity : AppCompatActivity(),KodeinAware, PodcastDetailsEventListener,
    ...
    private fun getPodcastDetails(podId: String) {
        lifecycleScope.launch {
            try {
                val res=homeRepositry.getPodcastDetails(userId, podId)
                if (res.status){
    ...
    fun onPodcastLike() {
        lifecycleScope.launch {
            try {
                val res=homeRepositry.managePodcastLikes(userId,podId)
                if (res.status){
                    getPodcastDetails(podId)
                }
            }catch (e: ApiException){
    ...
    private fun shareInsideApp() {
        ...
        lifecycleScope.launch {
            try {
                val res=homeRepositry.sharePodcasts(data)
                if (res.status){
    ...
    private fun reportThePost() {
        ...
        lifecycleScope.launch {
            try {
                val res=homeRepositry.spineReportUserPostStory(userId,podId,"5",reportTitle,reportReason,reportMessage)
                if (res.status){
