...
class StoryCommenReplytActivity : AppCompatActivity(), KodeinAware, StoryCommentReplyEventListener,
    ...
    private fun getCommentDetails() {
        data =
            intent.getSerializableExtra(PostCommentActivity.SPINE_COMMENT_DATA) as StoriesCommentData
        val COMMENT_PROFILE_IMAGE = intent.getStringExtra(PostCommentActivity.COMMENT_PROFILE_IMAGE)
        storyId = data.spineStoryId
        commentId = data.id
        getReplies(commentId)
        Glide.with(binding.circleImageView5).load(COMMENT_PROFILE_IMAGE + data.profilePic)
            .placeholder(R.drawable.ic_profile).into(binding.circleImageView5)
                ...
                if (res.status) {
                    ...
                    binding.rvCommentReply.also {
                        it.layoutManager = LinearLayoutManager(
                            this@StoryCommenReplytActivity,
    ...
    override fun onSend(reply: String) {
        lifecycleScope.launch {
            try {
                val res = eventRepository.spineStoriesComment(storyId, userId, commentId, reply)
                if (res.status) {
