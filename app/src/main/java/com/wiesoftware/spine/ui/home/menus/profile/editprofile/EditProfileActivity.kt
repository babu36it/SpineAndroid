...
//                binding.tvNameCounter.setText(40-s.length);
    ...
    override fun onSaveProfile(
        ...
        lifecycleScope.launch {
            try {
                showProgressDialog()
                val res = profileRepository.editProfille(
    ...
    private fun updateProfilePic() {
        ...
        lifecycleScope.launch {
            try {
                showProgressDialog()
                val res = profileRepository.updateUserProfilePic(facility_image)
                if (res.status) {
                    dismissProgressDailog()
                    Toast.makeText(this@EditProfileActivity, res.message, Toast.LENGTH_SHORT).show()
                } else {
    ...
    private fun updateBgProfilePic() {
        ...
        lifecycleScope.launch {
            try {
                showProgressDialog()
                val res = profileRepository.updateUserBgProfilePic(facility_image!!)
                if (res.status) {
                    dismissProgressDailog()
                    Toast.makeText(this@EditProfileActivity, res.message, Toast.LENGTH_SHORT).show()
                } else {
