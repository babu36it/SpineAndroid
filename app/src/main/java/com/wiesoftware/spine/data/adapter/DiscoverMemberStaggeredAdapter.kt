 ...
 */
class DiscoverMemberStaggeredAdapter(val postList: List<PostData>, val listener: OwnPostSelectedListener):RecyclerView.Adapter<DiscoverMemberStaggeredAdapter.OwnPostHolder>() {
...
//        holder.itemView.layoutParams = SpanLayoutParams(SpanSize(width, height))
        ...
        if (position == 4){
            url = "https://demonuts.com/Demonuts/SampleImages/W-03.JPG"

        }

        if (position == 5){
            url = "https://demonuts.com/Demonuts/SampleImages/W-08.JPG"
        }
        if (position == 6){
